...
class BFS {
 public:
  ...
  std::vector<Coord> findPath(Coord start, Coord end) {
    ...
    std::queue<Coord> open_list;
    open_list.push(start);
    map_[start.y][start.x].visited = true;
    while (!open_list.empty()) {
      auto current_corrd = open_list.front();
      open_list.pop();
      //! finish check
 ...
 private:
  ...
  void drawOpenList(const std::queue<Coord> &open_list) {
    auto op = open_list;
    while (!op.empty()) {
      auto coord = op.front();
      op.pop();
    ...
    cv::namedWindow("bfs", 0);
    cv::imshow("bfs", display_img_);
    cv::waitKey(1);
  }
